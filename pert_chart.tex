...
\begin{figure}
 	...
 	\node [block] (kickoff) {Kick-Off Meeting};
        \node [block] (qualityplan) {Quality Plan};
        \node [block] (riskplan) {Risk Management Plan};
        \node [block] (doristeam) {DORIS task-force};
        \node [block] (slrteam) {SLR task-force};
        \node [block] (vlbiteam) {VLBI task-force};
        \node [block] (recruitment) {PhD \& PostDoc recruitment};
